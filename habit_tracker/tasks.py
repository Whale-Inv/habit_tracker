...
def check_upcoming_habits():
    ...
    for habit in habits:
        ...
        if (
            habit.habit_time.date() == reminder_time.date()
            and habit.habit_time.hour == reminder_time.hour
            and habit.habit_time.minute == reminder_time.minute
        ):
            if habit.creator.telegram_chat_id:
                message = (
                    f"🔔 Через 15 минут: {habit.habit_action} в {habit.habit_place}"
                )
